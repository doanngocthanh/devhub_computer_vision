...
public class AuthService {
    ...
    public AuthService(DatabasePlugin db, @Value("${devhub.jwt.secret:devhub-secret-do-not-use-in-prod}") String jwtSecret,
    ...
    public boolean register(String email, String rawPassword, String firstName, String lastName) {
        ...
        int res = db.execute("INSERT INTO users(email, password_hash, first_name, last_name, created_at) VALUES(:e, :p, :f, :l, :c)",
                Map.of("e", email, "p", hash, "f", firstName, "l", lastName, "c", Instant.now().toString()));
        return res > 0;
    }

    public String authenticate(String email, String rawPassword) {
        List<Map<String, Object>> rows = db.query("SELECT id, password_hash FROM users WHERE email = :e", Map.of("e", email));
        if (rows == null || rows.isEmpty()) return null;
        Map<String, Object> row = rows.get(0);
        String hash = String.valueOf(row.get("password_hash"));
        if (!passwordEncoder.matches(rawPassword, hash)) return null;
