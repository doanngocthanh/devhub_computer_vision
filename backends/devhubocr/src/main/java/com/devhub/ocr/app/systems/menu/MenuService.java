...
public class MenuService {
    ...
    public boolean createMenu(String title, String icon, String path, Integer parentId, String rolesCsv,
            Integer orderNum, Boolean isActive) {
    ...
    public List<Map<String, Object>> getMenuTree() {
        ...
        // Step 2: Link children vào parent
        for (Map<String, Object> r : all) {
            Integer id = parseInteger(r.get("id"));
            Integer pid = parseInteger(r.get("parent_id"));

            if (id == null || pid == null || pid == 0)
                continue;
            ...
            if (parent != null) {
                ...
                // Đánh dấu id này KHÔNG phải root vì nó có parent
                nonRootIds.add(id);
            }
        }

        // Step 3: Root = những node không nằm trong nonRootIds
        List<Map<String, Object>> roots = new ArrayList<>();
        for (Integer id : byId.keySet()) {
            if (!nonRootIds.contains(id)) {
                roots.add(byId.get(id));
            }
        }
